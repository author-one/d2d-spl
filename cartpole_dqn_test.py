...
from cartpole_dqn_pytorch import DQN
...
if __name__ == '__main__':
    ...
    for trial in range(num_trials):
        env.seed(trial)
        model_path = 'pytorch_dqn_results/model' + str(trial).zfill(2) + '-1000.p'
    ...
    for trial in range(num_trials):
        env.seed(trial)
        model_path = 'pytorch_dqn_results/model' + str(trial).zfill(2) + '-2000.p'
        dqn = pickle.load(open(model_path, "rb"))
        dqn.exploration_rate = 0.0
        test_dqn(trial, dqn, env)
